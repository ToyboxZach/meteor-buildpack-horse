...
if [[ -n "$HEROKU_METRICS_URL" ]] && [[ "${DYNO}" != run\.* ]] && [[ -z "$HEROKU_SKIP_NODE_PLUGIN" ]]; then
  ...
  if [[ -z "$NODEJS_PARAMS" ]]; then
      export NODEJS_PARAMS="--require $HOME/.heroku/metrics/metrics_collector.cjs"
  else
      export NODEJS_PARAMS="${NODEJS_PARAMS} --require $HOME/.meteor/heroku_build/metrics/metrics_collector.cjs"
  fi
